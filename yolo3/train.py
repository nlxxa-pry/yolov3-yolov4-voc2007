...
def fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda,writer):
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            niter = iteration + epoch*epoch_size
            ...
            writer.add_scalars('Loss', {'Train_loss':loss.item()}, niter)
    ...
    writer.add_scalars('Loss', {'val_loss': val_loss/(epoch_size_val+1)}, niter)
    print('Saving state, iter:', str(epoch+1))
    torch.save(model.state_dict(), 'log30/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth'%((epoch+1),total_loss/(epoch_size+1),val_loss/(epoch_size_val+1)))
    return writer
...
if __name__ == "__main__":
    ...
    if True:
        ...
        for epoch in range(Init_Epoch,Freeze_Epoch):
            writer = fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,gen_val,Freeze_Epoch,Cuda,writer)
            lr_scheduler.step()
    ...
    if True:
        ...
        for epoch in range(Freeze_Epoch,Unfreeze_Epoch):
            writer = fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,gen_val,Unfreeze_Epoch,Cuda,writer)
            lr_scheduler.step()
